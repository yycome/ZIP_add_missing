...
# Code: Step 1. Identify missing zipcodes in Mountain                                  #
# Author: Yaguang Wei                                                                  #
########################################################################################
...
dir_grid_urban <- '/media/qnap4/heresh-predictions-combined/8 Mountain/Urban areas at 50m spatial resolution/'
dir_grid_nonurban <- '/media/qnap4/heresh-predictions-combined/8 Mountain/Non-urban areas at 1km spatial resolution/'
...
############################# 1. br ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'br_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(br_zip_to_add_tmp,file=paste0(dir_save_tmp,'br_20',i,'_M.rds'))
...
############################# 2. ca ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'ca_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(ca_zip_to_add_tmp,file=paste0(dir_save_tmp,'ca_20',i,'_M.rds'))
...
############################# 3. cu ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'cu_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(cu_zip_to_add_tmp,file=paste0(dir_save_tmp,'cu_20',i,'_M.rds'))
...
############################# 4. ec ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'ec_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(ec_zip_to_add_tmp,file=paste0(dir_save_tmp,'ec_20',i,'_M.rds'))
...
############################# 5. fe ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'fe_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(fe_zip_to_add_tmp,file=paste0(dir_save_tmp,'fe_20',i,'_M.rds'))
...
############################# 6. k ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'k_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(k_zip_to_add_tmp,file=paste0(dir_save_tmp,'k_20',i,'_M.rds'))
...
############################# 7. nh4 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'nh4_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(nh4_zip_to_add_tmp,file=paste0(dir_save_tmp,'nh4_20',i,'_M.rds'))
...
############################# 8. ni ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'ni_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(ni_zip_to_add_tmp,file=paste0(dir_save_tmp,'ni_20',i,'_M.rds'))
...
############################# 9. no3 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'no3_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(no3_zip_to_add_tmp,file=paste0(dir_save_tmp,'no3_20',i,'_M.rds'))
...
############################# 10. oc ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'oc_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(oc_zip_to_add_tmp,file=paste0(dir_save_tmp,'oc_20',i,'_M.rds'))
...
############################# 11. pb ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'pb_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(pb_zip_to_add_tmp,file=paste0(dir_save_tmp,'pb_20',i,'_M.rds'))
...
############################# 12. si ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'si_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(si_zip_to_add_tmp,file=paste0(dir_save_tmp,'si_20',i,'_M.rds'))
...
############################# 13. so4 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'so4_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(so4_zip_to_add_tmp,file=paste0(dir_save_tmp,'so4_20',i,'_M.rds'))
...
############################# 14. v ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'v_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(v_zip_to_add_tmp,file=paste0(dir_save_tmp,'v_20',i,'_M.rds'))
...
############################# 15. z ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save_tmp,'z_20',i,'_M.rds'))){
    ### aggregated file
    ...
    saveRDS(z_zip_to_add_tmp,file=paste0(dir_save_tmp,'z_20',i,'_M.rds'))
